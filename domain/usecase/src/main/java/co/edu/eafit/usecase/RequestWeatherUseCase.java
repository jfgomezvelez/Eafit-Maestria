...
public class RequestWeatherUseCase {
    ...
    private final WeatherIntegrationRepository weatherIntegrationRepository;

    public Mono<Weather> requestWeather(String location, String processId) {
        ...
        return weatherRepository.checkWeather(location)
                .flatMap(weather -> {
                    Process processFinish = processInitial
                            ...
                            .build();
                    return weatherIntegrationRepository.requestWeather(weather, processId)
                            .flatMap(weatherSent -> statisticRepository.save(processFinish).map(result -> weatherSent));
                });
    }
}
